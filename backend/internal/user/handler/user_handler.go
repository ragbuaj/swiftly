...
import (
	...
	"swiftly/backend/internal/pkg/validator"
...
func (h *UserHandler) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	tokens, err := h.service.CreateUser(r.Context(), req, ip, userAgent)
	if err != nil {
...
func (h *UserHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	ip := r.RemoteAddr
	userAgent := r.UserAgent()
	tokens, err := h.service.Login(r.Context(), req, ip, userAgent)
	if err != nil {
		response.HandleError(w, err)
		return
	}
...
func (h *UserHandler) RefreshToken(w http.ResponseWriter, r *http.Request) {
	...
	ip := r.RemoteAddr
	userAgent := r.UserAgent()
	tokens, err := h.service.RefreshToken(r.Context(), refreshStr, ip, userAgent)
	if err != nil {
		response.HandleError(w, err)
		return
	}
...
func (h *UserHandler) ForgotPassword(w http.ResponseWriter, r *http.Request) {
	...
	token, email, err := h.service.ForgotPassword(r.Context(), req.Identifier)
	if err != nil {
...
func (h *UserHandler) Logout(w http.ResponseWriter, r *http.Request) {
	...
	if token != "" { h.service.Logout(r.Context(), token) }
...
func (h *UserHandler) SocialCallback(w http.ResponseWriter, r *http.Request) {
	...
	ip := r.RemoteAddr
	userAgent := r.UserAgent()
	tokens, err := h.service.SocialLogin(r.Context(), socialUser, ip, userAgent)
	if err != nil {
		response.HandleError(w, err)
		return
	}
...
func (h *UserHandler) GetSessions(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil { sessionID = claims.SessionID }

	sessions, err := h.service.GetActiveSessions(r.Context(), userID, sessionID)
	if err != nil {
		response.HandleError(w, err)
		return
	}
...
func (h *UserHandler) VerifyOTP(w http.ResponseWriter, r *http.Request) {
	...
	err := h.service.VerifyOTP(r.Context(), req.Email, req.OTP)
	if err != nil {
		response.HandleError(w, err)
		return
	}
...
func (h *UserHandler) ResendOTP(w http.ResponseWriter, r *http.Request) {
	...
	otp, err := h.service.GenerateAndStoreOTP(r.Context(), req.Email)
	if err != nil {
		response.HandleError(w, err)
		return
	}
...
func (h *UserHandler) ValidateResetToken(w http.ResponseWriter, r *http.Request) {
	...
	err := h.service.ValidateResetToken(r.Context(), token)
	if err != nil {
		response.HandleError(w, err)
		return
	}
...
func (h *UserHandler) GoogleTokenLogin(w http.ResponseWriter, r *http.Request) {
	...
	ip := r.RemoteAddr
	userAgent := r.UserAgent()
	tokens, err := h.service.LoginWithGoogleToken(r.Context(), req.IDToken, ip, userAgent)
	if err != nil {
		response.HandleError(w, err)
		return
	}
